    ...
    >
      ...
      <div class="attachment-panel" v-if="attachment" id="view-attachment">
        ...
        <div class="attachment-info">
          ...
          <div class="item">
            需要<span class="points">{{ attachment.points }}</span
            >积分
          </div>
...
const downloadAttachment = async (fileId) => {
  ...
  //判断用户积分是否够
  if (result.data.Points < attachment.value.points) {
    proxy.Message.warning("你的积分不够，无法下载");
    return;
  }

  proxy.Confirm(
    `你还有${result.data.userPoints}积分，当前下载会扣除${attachment.value.points}积分，确定要下载吗？`,
    () => {
      downloadDo(fileId);
    }
  );
